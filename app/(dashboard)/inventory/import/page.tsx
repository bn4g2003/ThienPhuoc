...
export default function Page() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(`/api/inventory/import?warehouseId=${selectedWarehouseId}`);
      const body = await res.json();
      return body.success ? body.data : [];
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["inventory", "import", selectedWarehouseId] });
    },
  });
  ...
  return (
    <>
      <WrapperContent<ImportTransaction>
        isLoading={isLoading}
        header={{
          refetchDataWithKeys: selectedWarehouseId ? ["inventory", "import", String(selectedWarehouseId)] : ["inventory", "import"],
          customToolbar: (
